...
public:
	...
	void Reset()
	{
		...
		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				temp->SetNumber(0); //Reset to zero
				temp = temp->GetRight();
			}
			if (temp_sub->GetDown() == NULL)
				break;
			temp = temp_sub->GetDown();
			temp_sub = temp_sub->GetDown();
		}
	...
	void Turn_Right()
	{
		...
		MyBlock* temp_move = new MyBlock;

		m_pHead = m_pHead->GetDown()->GetDown()->GetDown();
		temp_move = m_pHead;
		...
		for (int i = 0; i < 4; i++)
		{
			...
			for (int k = 0; k < i; k++)
			{
				temp_move = temp_move->GetDown();
			}

			for (int j = 0; j < 4; j++)
			{
				temp = temp_move->GetUp();
				temp_move->SetUp(temp_move->GetLeft());
				temp_move->SetLeft(temp_move->GetDown());
				temp_move->SetDown(temp_move->GetRight());
				temp_move->SetRight(temp);
				temp_move = temp_move->GetRight();
