...
BpTreeNode* BpTree::searchDataNode(int n) {
	...
	while (temp->getMostLeftChild() != NULL) { // exist leftchild 

		iter = dynamic_cast<BpTreeIndexNode*> (temp)->getIndexMap()->begin();
		...
		if (n < iter->first) {  // indexnode
			...
			while (true) {
				iter++; 
				if (iter == dynamic_cast<BpTreeIndexNode*>(temp)->getIndexMap()->end() || n < iter->first) {
					iter--;
					temp = iter->second;
					break;
				}
